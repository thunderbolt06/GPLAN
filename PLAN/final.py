...
#         self.control.destroy()
    ...
    def reset(self):
        ...
        self.root.destroy()
        self.clear()
        self.draw(cell_size = 30)

    def instructions(self):
        tk.messagebox.showinfo("Instructions",
        "----------User Instructions----------\n1. All rooms are to be drawn in a clockwise manner.\n2. Kindly ensure that the start point and end point are same for each room while drawing the rooms.\n3. After partitioning, only the partitions of orthogonal rooms need to be labelled as per user's convenience and rectangular rooms are implicitly labelled.\n4. Minimum width and height for rooms are to be entered in the order of room labels.")
    ...
    def draw(self, cell_size = 50):
        ...
        self.frame = tk.Frame(self.root, height=1000, width=500)
        self.frame.pack(fill="both", expand=True)

        self.c = tk.Canvas(self.frame, height=500, width=500, bg='white')
        ...
        self.instru = tk.Button(self.frame, text="Instructions",relief = 'raised' ,command=self.instructions)
        self.instru.pack(side="left",padx = (5,5), pady = (5,0))

        self.ex = tk.Button(self.frame, text="Exit",relief = 'raised' ,command=self.exit)
        self.ex.pack(side="right",padx = (5,5), pady = (5,0))

        self.re = tk.Button(self.frame, text="Reset",relief = 'raised' ,command=self.reset)
        self.re.pack(side="left",padx = (5,5), pady = (5,0))

        self.title_var = tk.StringVar(self.frame,value="Draw Layout")
        self.label = tk.Label(self.frame, textvariable=self.title_var, relief=tk.FLAT)
        self.label.pack(padx = (20,5), pady = (5,0))
...
class FloorPlan:
    ...
    def draw_rfp(self, orthogonal_rooms, ax, draw_partitions=False):
        ...
        ax.invert_yaxis()
        if draw_partitions:
        	ax.title.set_text('Dimensioned Floorplan with Labeled Partitions')
        else:
        	ax.title.set_text('Dimensioned Floorplan')
...
def gui_fnc(nodes):
    ...
    for i in range(0,nodes):
        ...
        if(i_value_y == 0):
            ...
            text_head_area[i_value_x].place(relx = 0.40 + 0.20*i_value_x,  
                      rely = 0.2, 
                      anchor ='ne')
        ...
        text_room[i] = tk.Label(root, text ="Room"+str(i+1),font = ("Times New Roman",8)) 
        ...
        value_area[i].place(relx = 0.40+ 0.20*i_value_x,  
                      rely = 0.3 + (0.05)*i_value_y, 
                       anchor ='ne')
...
def digraph_to_eq(VER, HOR, user_rooms,total_rooms):
	...
	[inp_min_width,inp_min_height] = gui_fnc(user_rooms)
	...
	for i in range(total_rooms-user_rooms):
		inp_min_width.append(0)
		inp_min_height.append(0)
	

	b_VER = np.dot(np.array(inp_min_width),-1)
	b_VER = np.transpose(b_VER)
	b_VER = b_VER.astype(float)
	dimensions = solve_linear(N,f_VER, A_VER, b_VER, Aeq_VER, Beq_VER, f_HOR, A_HOR, Aeq_HOR, Beq_HOR, inp_min_height)
...
def solve_linear(N,f_VER, A_VER, b_VER, Aeq_VER, Beq_VER, f_HOR, A_HOR, Aeq_HOR, Beq_HOR, inp_min_height):
	...
	b_HOR = np.dot(np.array(inp_min_height),-1)
